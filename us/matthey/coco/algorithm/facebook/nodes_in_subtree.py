...
def count_of_subnode_helper(root, values):
    res = 1 if root.val in values else 0
    for c in root.children:
        res += count_of_subnode_helper(c, values)
    return res


def find_root_helper(root, val):
    if root.val == val:
        return root
    else:
        for c in root.children:
            found = find_root_helper(c, val)
...
def count_of_nodes(root, queries, s):
    ...
    res = []
    for q in queries:
        values = [i + 1 for i in range(len(s)) if s[i] == q[1]]
        newroot = find_root_helper(root, q[0])
        res.append(count_of_subnode_helper(newroot, values))
    return res
